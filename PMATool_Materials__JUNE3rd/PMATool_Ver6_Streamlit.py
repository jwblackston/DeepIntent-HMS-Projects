...
def find_closest_audience_size(data, seed_size, tolerance=0.01):  # 1% tolerance by default
...
def find_threshold(dry_run_data, closest_size, tolerance=0.01):  # 1% tolerance by default
...
#After submit is hit, run all the functions defined and return the resulting rows
    ...
    if 'related_stats' in st.session_state:
        ...
        st.write(f"Audience size: {closest_audience_size}, Threshold: {threshold}, Model power index: {model_power}, AQ score: {aq_score}, Cohort seed size: {seed_size}.")
